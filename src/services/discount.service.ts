...
export const getDiscountRuleById = async (
  id: number
): Promise<DiscountRuleResponse | null> => {
  try {
...
export const getDeletedDiscountRules = async (query: {
  page?: number;
  limit?: number;
  sortBy?: string;
  sortOrder?: string;
...
export const restoreDiscountRule = async (
  id: number
): Promise<DiscountRuleResponse> => {
  try {
    const rule = await prisma.discountRule.findUnique({ where: { id } });
    if (!rule)
      throw new Error(`Discount rule with ID ${id} not found or not deleted`);
  ...
  try {
    ...
    if (rule.type === "DIRECT_PERCENTAGE") {
      discountAmount = Math.min(
        ...
        rule.maxDiscountAmount || Infinity
      );
    } else if (rule.type === "DIRECT_NOMINAL") {
      discountAmount = rule.value!;
    } else if (rule.type === "BOGO") {
      if (!rule.productId)
        throw new Error("BOGO discount must be associated with a product");
